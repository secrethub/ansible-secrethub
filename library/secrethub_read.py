from ansible.module_utils.client import CLIInaccessible, ReadError
from ansible.module_utils.secrethub_base import BaseModule

DOCUMENTATION = '''
---
module: secrethub_read

short_description: Reads a secret.
...
RETURN = '''
secret:
  description: The secret value stored in the given path.
  type: str
'''
...
class ReadModule(BaseModule):
    ...
    def run(self):
        try:
            self.exit_json(
                changed=False,
                secret=self.client().read(self.params.get('path')),
            )
        except (CLIInaccessible, ReadError) as e:
            self.fail_json(
                changed=False,
                msg=str(e),
            )
